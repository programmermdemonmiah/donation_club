...
const navigation = [
    ...
    { label: 'Donations', href: 'deposits.index', icon: 'M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33' },
    ...
    { label: 'Transfer', href: 'transfer.create', icon: 'M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5' },
    ...
    { label: 'Community Support', href: 'returns.index', icon: 'M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z' },
...
export default function AppLayout({ children }: PropsWithChildren) {
    ...
    const NavLink = ({ item }: { item: typeof navigation[0] }) => {
        ...
        return (
            ...
            >
                <svg className={cn('h-5 w-5 shrink-0', active ? 'text-white' : 'text-gray-500 group-hover:text-emerald-400')} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    return (
        <div className="flex min-h-screen bg-white">
            {/* ── Sidebar ── */}
            <aside className={cn(
                'fixed inset-y-0 left-0 z-40 flex flex-col border-r border-blue-900/50 bg-blue-950 transition-all duration-300 lg:translate-x-0',
                sidebarOpen ? 'translate-x-0' : '-translate-x-full',
                isCollapsed ? 'w-[70px]' : 'w-[240px]'
            )}>
                {/* Logo */}
                <div className={cn('flex h-16 shrink-0 items-center border-b border-blue-900/50', isCollapsed ? 'justify-center px-0' : 'justify-between px-5')}>
                    <Link href={route('home')} className={cn('flex items-center gap-2.5', isCollapsed && 'hidden')}>
                        <div className="h-9 w-9 overflow-hidden rounded-xl bg-gradient-to-br from-blue-600 to-emerald-500 shadow-[0_0_15px_rgba(37,99,235,0.35)]">
                    ...
                    <button
                        ...
                        className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-blue-900/50 hover:text-white"
                    >
                ...
                <nav className="flex-1 space-y-0.5 overflow-y-auto px-3 py-4">
                    ...
                    <div className="my-4 border-t border-blue-900/50" />
                    {!isCollapsed && (
                ...
                {/* User footer */}
                <div className={cn('border-t border-blue-900/50 p-3', isCollapsed && 'flex justify-center')}>
                    <button
                        onClick={logout}
                        title={isCollapsed ? 'Sign out' : undefined}
                        className={cn('flex w-full items-center rounded-xl px-3 py-2.5 text-sm font-semibold text-gray-400 transition-all hover:bg-blue-900/50 hover:text-red-400', isCollapsed ? 'justify-center' : 'gap-3')}
                    >
            ...
            {/* Mobile overlay */}
            {sidebarOpen && (
                <div className="fixed inset-0 z-30 bg-blue-950/60 backdrop-blur-sm lg:hidden" onClick={() => setSidebarOpen(false)} />
            )}
                ...
                <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-gray-100 bg-white/95 px-4 backdrop-blur-sm sm:px-6 lg:px-8">
                    ...
                    <div className="ml-auto flex items-center gap-3">
                        ...
                        <Link
                            ...
                            className="hidden items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-black text-white shadow-[0_0_15px_rgba(37,99,235,0.25)] transition-all hover:bg-blue-500 hover:shadow-[0_0_25px_rgba(37,99,235,0.4)] sm:flex"
                        >
                        ...
                        <div className="relative">
                            ...
                            >
                                <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-100 text-xs font-black text-blue-700">
                                    ...
                                    <div className="absolute right-0 z-20 mt-2 w-56 origin-top-right overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-xl shadow-gray-200/60">
                                        ...
                                        {[
                                            ...
                                            <Link key={item.label} href={item.href} onClick={() => setProfileOpen(false)}
                                                className="block px-4 py-2.5 text-sm font-medium text-gray-700 transition-colors hover:bg-blue-50 hover:text-blue-700">
                                                {item.label}
                                            </Link>
                                        ))}
