...
const navigation = [
    ...
    { label: 'Donations', route: 'admin.commissions.index', icon: 'M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.42 3.68 3.83V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z' },
    { label: 'Community Support', route: 'admin.returns.index', icon: 'M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z' },
    ...
    { label: 'Generate PIN', route: 'admin.pins.index', icon: 'M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm5 11h-4v4h-2v-4H7v-2h4V7h2v4h4v2z' },
    ...
    { label: 'Hero Section', route: 'admin.hero.edit', icon: 'M4 6h16v12H4zM2 4v16h20V4H2zm9 7H7v4h4v-4zm6-2h-4v6h4V9z' },
...
export default function AdminLayout({ children }: PropsWithChildren) {
    ...
    return (
        <div className="flex min-h-screen bg-blue-950/[0.02]">
            <aside
                className={cn(
                    'fixed inset-y-0 left-0 z-40 flex flex-col bg-blue-950 transition-all duration-300 lg:translate-x-0',
                    sidebarOpen ? 'translate-x-0' : '-translate-x-full',
                    isCollapsed ? 'w-20' : 'w-60'
                )}
            >
                <div className={cn("flex h-16 shrink-0 items-center justify-between border-b border-blue-900/50", isCollapsed ? "px-0 justify-center" : "px-5")}>
                    <Link href={route('home')} className={cn("flex items-center gap-2 text-base font-bold text-white", isCollapsed && "hidden")}>
                        <div className="h-8 w-8 overflow-hidden rounded-lg bg-blue-500 flex items-center justify-center">
                    ...
                    <button
                        ...
                        className="rounded-md p-1.5 text-slate-400 hover:bg-blue-900/60 hover:text-white"
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    ...
                                    active ? 'bg-blue-600 text-white' : 'text-slate-300 hover:bg-blue-900/60 hover:text-white',
                ...
                <div className="shrink-0 border-t border-blue-900/50 px-3 py-3">
                    <Link
                        href={route('dashboard')}
                        title={isCollapsed ? "Member View" : undefined}
                        className={cn("flex items-center rounded-lg px-3 py-2 text-sm font-medium text-slate-400 hover:bg-blue-900/60 hover:text-white", isCollapsed ? "justify-center" : "gap-3")}
                    >
                        <svg className="h-5 w-5 shrink-0 fill-current" viewBox="0 0 24 24">
            ...
            {sidebarOpen && <div className="fixed inset-0 z-30 bg-blue-950/40 lg:hidden" onClick={() => setSidebarOpen(false)} />}
                    ...
                    <div className="ml-auto flex items-center gap-4">
                        <div className="relative pl-4">
                            <button
                                ...
                                className="flex items-center gap-2 rounded-full text-sm hover:ring-2 hover:ring-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                            >
                                ...
                                <svg className="hidden h-4 w-4 text-slate-400 sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
