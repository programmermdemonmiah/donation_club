...
export default function Kyc() {
    ...
    const statusColor = (s: string) => s === 'verified' || s === 'approved' ? 'emerald' : s === 'pending' ? 'blue' : s === 'rejected' ? 'rose' : 'gray';

    return (
        <AppLayout>
            <div className="mb-8">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-600">Identity</p>
            ...
            {/* Status banner */}
            <div className={`mb-6 flex items-center gap-4 rounded-2xl border p-5 ${kycStatus === 'verified' ? 'border-emerald-200/50 bg-emerald-50' : kycStatus === 'pending' ? 'border-blue-200/50 bg-blue-50' : 'border-gray-100 bg-white'} shadow-sm`}>
                <div className={`flex h-12 w-12 items-center justify-center rounded-xl ${kycStatus === 'verified' ? 'bg-emerald-100' : kycStatus === 'pending' ? 'bg-blue-100' : 'bg-gray-100'}`}>
                    <svg className={`h-6 w-6 ${kycStatus === 'verified' ? 'text-emerald-600' : kycStatus === 'pending' ? 'text-blue-600' : 'text-gray-400'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <form onSubmit={submit} className="space-y-4">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <input ref={fileRef} type="file" accept=".jpg,.jpeg,.png,.pdf"
                                        ...
                                        className="block w-full text-sm text-gray-500 file:mr-4 file:rounded-xl file:border-0 file:bg-blue-50 file:px-4 file:py-2 file:text-sm file:font-black file:text-blue-700 hover:file:bg-blue-100" />
