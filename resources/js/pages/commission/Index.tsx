...
export default function Donations() {
    ...
    const stats = [
        { label: 'Total Donations', value: formatMoney(page.props.totals.completed), accent: 'emerald' },
        ...
        ...Object.entries(page.props.byGeneration).slice(0, 2).map(([gen, total]) => ({
            label: Number(gen) === 1 ? 'Direct Donations' : `Generation ${gen}`,
            value: formatMoney(total), accent: 'blue',
        })),
    ];
    ...
    return (
        <AppLayout>
            <div className="mb-8">
                <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-600">Donations</p>
                <h1 className="mt-1.5 text-3xl font-black tracking-tight text-gray-900">Donations</h1>
            </div>
            ...
            <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {stats.map((s) => (
                    <div key={s.label} className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                        ...
                        <p className={`mt-2 text-2xl font-black ${s.accent === 'emerald' ? 'text-emerald-600' : s.accent === 'blue' ? 'text-blue-600' : 'text-gray-700'}`}>
                            {s.value}
                        </p>
                    </div>
                ))}
                ...
                <Table<CommissionRow>
                    columns={[
                        ...
                        { header: 'Generation', render: (c) => (
                            <span className={`rounded-full px-2 py-0.5 text-xs font-black ${c.generation === 1 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}>
                        ...
                        { header: 'Donated', render: (c) => <span className="text-sm font-black text-emerald-600">{formatMoney(c.amount)}</span> },
                    ...
                    rowKey={(c) => c.id}
                    emptyMessage="No referral donations yet. Donations are recorded automatically when enabled."
                />
