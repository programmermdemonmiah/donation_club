...
function StatCard({ label, value, sub, icon, accent = 'blue' }: {
    ...
    icon: string; accent?: 'blue' | 'emerald' | 'green' | 'rose';
}) {
    const colors = {
    ...
    const textColors = {
        ...
        green: 'text-emerald-600', rose: 'text-rose-600',
    };
    return (
        ...
        <AppLayout>
            ...
            <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Link
                    ...
                    className="group relative inline-flex items-center gap-2 overflow-hidden rounded-xl bg-blue-600 px-6 py-3 text-sm font-black text-white shadow-[0_0_20px_rgba(37,99,235,0.25)] transition-all duration-300 hover:bg-blue-500 hover:shadow-[0_0_35px_rgba(37,99,235,0.4)]"
                >
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <StatCard label="Available Balance" value={formatMoney(stats.balance)} sub={`${formatMoney(stats.locked_balance)} locked`} accent="blue"
                ...
                <StatCard label="Total Donate" value={formatMoney(stats.total_deposit)} accent="blue"
                ...
                <StatCard label="Total Transfer" value={formatMoney(stats.total_transfer)} accent="blue"
                    icon="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" />
                <StatCard label="Total Income" value={formatMoney(stats.total_return)} accent="emerald"
                    icon="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                <StatCard label="Total Generation" value={formatMoney(stats.total_commission)} accent="emerald"
                ...
                <StatCard label="Current Rank" value={stats.current_rank ?? '—'} accent="blue"
            ...
            <div className="mt-8 overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm shadow-gray-200/50">
                <div className="flex items-center justify-between border-b border-gray-50 px-6 py-5">
                    ...
                    <Link href={route('transactions.index')} className="rounded-lg border border-gray-100 bg-gray-50 px-3 py-1.5 text-xs font-bold text-gray-600 transition-all hover:border-blue-200 hover:bg-blue-50 hover:text-blue-700">
                        View all →
                    </Link>
                </div>
                <Table<Tx>
                    ...
                    rowKey={(t) => t.id}
                    emptyMessage="No wallet activity yet — donations and withdrawals appear here."
                />
            </div>
        </AppLayout>
    );
